...
Schools_list_names = list(str(i["name"]) for i in Schools_list)
...
def send_send(message):
    ...
    if str(message.text) in Schools_list_ids:
        ...
        bot.send_message(message.from_user.id,
                         f"Вы присоединились к {Schools_list_names[Schools_list_ids.index(users[user_id]['Id_School'])]}",
                         reply_markup=markup)
        return
...
def Choose_School(message):
    ...
    if str(message.text) == "Все меню":
        ...
        button = types.KeyboardButton("Отмена")
        markup.add(button)
        bot.send_message(message.from_user.id, "Напишите название блюда, которое вы хотите изменить",
                         reply_markup=markup)
...
def Set_dish_name(message):
    ...
    bot.send_message(message.from_user.id, "Напишите цену блюда")
    bot.register_next_step_handler(message, Set_dish_count)


def Set_dish_count(message):
    if str(message.text).strip() == "Отмена":
        close_dish(message)
        return
    user_id = message.from_user.id
    ...
    bot.send_message(message.from_user.id, f"Напишите \"Да\", если блюдо есть в наличии, или \"Нет\", если нет")
    bot.register_next_step_handler(message, Set_dish_isHere)


def Set_dish_isHere(message):
    if str(message.text).strip() == "Отмена":
        close_dish(message)
        return
    ...
    requests.get(
        f'https://12447695.pythonanywhere.com/api/dishes'
        f'/{users[user_id]["Id_School"]}/set/{users[user_id]["Dish"]["Name"].capitalize()}/{users[user_id]["Dish"]["Count"]}/{isHere}')
    bot.send_message(message.from_user.id, "Блюдо было успешно изменино", reply_markup=users[user_id]["Markup"])
